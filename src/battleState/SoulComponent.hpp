...
public:
    ...
    void setStatus(int s) { status_ = s; } //-1为菜单状态，0为普通状态，1为重力状态
...
private:
    ...
    float setPositionSmooth_ = true;
    float setPositionFactor_ = 0;
    float targetX_ = 0; float targetY_ = 0;
...
inline void SoulComponent::setPosition(float x, float y, bool smooth, float factor) {
    smooth ? targetX_ = x , targetY_ = y : x_ = x, y_ = y; // 这个表达式还挺好玩的awa
    smooth ? setPositionSmooth_ = true, setPositionFactor_ = factor : setPositionSmooth_ = false;
}
...
inline void SoulComponent::update(const BoxComponent& box) {
    ...
    if (setPositionSmooth_) {
        x_ += (targetX_ - x_) * setPositionFactor_;
        y_ += (targetY_ - y_) * setPositionFactor_;
    }
